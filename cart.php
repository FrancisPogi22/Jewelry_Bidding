...
<div class="col-lg-12">
	...
	<div class="row">
		<div class="col-md-8">
				...
				<ul class="list-group">
					...
					?>
						<li class="list-group-item" data-id="<?php echo $row['id'] ?>" data-price="<?php echo $row['price'] ?>">
							<div class="d-flex w-100 widget">
								<input type="checkbox" class="order-checkbox" data-price="<?php echo $row['price'] ?>" data-product-id="<?php echo $row['id'] ?>">
			...
			<?php else : ?>
				<center><img src="ecart.png" width="600px" height="400px" alt="emptycart"></center>
			<?php endif; ?>
		</div>
		<div class="col-md-4">
			<div class="card mb-4">
				...
				<div class="card-body">
					<h4 class="text-right"><b id="tamount">0</b></h4>
				</div>
			</div>
...
<script>
	function calculateTotalPrice() {
		let total = $('.order-checkbox:checked').toArray().reduce((acc, checkbox) => {
			let price = $(checkbox).data('price');
			let quantity = $(checkbox).closest('.widget').find('.qty-input').val();
			return acc + price * quantity;
		}, 0);

		$('#tamount').text(total.toLocaleString('en-US', {
			minimumFractionDigits: 2
		}));
	}

	$('.order-checkbox, .qty-input').change(calculateTotalPrice);
	...
	$('#checkout').click(function() {
		if ($('.order-checkbox:checked').length > 0) {
			...
			alert_toast('Please select at least one item to checkout.', "warning");
		}
	});
</script>