					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								...
								$cat = array();
								$cat[] = '';
								$qry = $conn->query("SELECT * FROM auction_categories ");
								while($row = $qry->fetch_assoc()){
									$cat[$row['id']] = $row['name'];
								}
								$books = $conn->query("SELECT b.*, u.firstname as uname, p.name as pname, ap.bid_end_datetime as bdt
                                FROM bids b
                                INNER JOIN users u ON u.id = b.user_id
                                INNER JOIN products p ON p.id = b.product_id
                                INNER JOIN auction_products ap ON ap.id = p.id");
								?>
								<?php if ($row!== null):?>
  <tr>
