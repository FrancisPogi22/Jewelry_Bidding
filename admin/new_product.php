        ...
        <div class="row">
          ...
          <!-- <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="text" class="form-control form-control-sm text-right number" name="productPrice" value="<?php echo isset($price) ? number_format($price) : '' ?>">
            </div>
          </div> -->
        </div>
        <div class="row">
            ...
            <?php else : ?>
              <div class="form-group">
                <div class="input-group mb-3">
                  ...
                  <input type="text" class="form-control" name="price[]" placeholder="Price Size">
                  <div class="input-group-append">
                    <span class="input-group-text"><a href="javascript:void(0)" onclick="$(this).closest('.form-group').remove()"><i class="fas fa-times"></i></a></span>
                  </div>
                </div>
...
<script>
  $('#default-preview .delete').click(function() {
    ...
    if ($('.inp-file[data-uuid="' + uuid + '"]').length > 0) {
      let fname = $('.inp-file[data-uuid="' + uuid + '"]').val()
      $.ajax({
  ...
  $(function() {
    $('#add_size').click(function() {
      let fg_inp = $('<div class="form-group"><div class="input-group mb-3">' +
        '<input type="hidden" class="form-control" name="sid[]">' +
        '<input type="text" class="form-control" name="size[]" placeholder="Size">' +
        '<input type="text" class="form-control" name="price[]" placeholder="Price Size">' +
        '<div class="input-group-append">' +
    ...
    myDropzone.on("addedfile", function(file) {
      ...
      file.previewElement.querySelector(".delete").onclick = function() {
        ...
        if ($('.inp-file[data-uuid="' + file.upload.uuid + '"]').length > 0) {
          let fname = $('.inp-file[data-uuid="' + file.upload.uuid + '"]').val()
          $.ajax({
            url: 'ajax.php?action=remove_file',
            method: 'POST',
            data: {
    ...
    myDropzone.on("success", function(file, resp) {
      if (resp) {
        ...
        if (resp.status == 1) {
          let inp = $('<input type="hidden" class="inp-file" name="fname[]" value="' + resp.fname + '" data-uuid="' + file.upload.uuid + '">')
          $('#f-inputs').append(inp)
