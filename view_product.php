...
<div class="col-lg-12">
      ...
      <div class="row">
        ...
        <div class="col-12 col-sm-6">
          ...
          while ($row = $sizes->fetch_assoc()) {
            $size_arr[$row['id']] = ['size' => $row['size'], 'price' => $row['price']];
          }
          ?>
            ...
            <select name="size_id" id="size_id" class="custom-select">
              ...
              <?php else : ?>
                <?php foreach ($size_arr as $k => $v) : ?>
                  <option value="<?php echo $k ?>" data-price="<?php echo $v['price'] ?>"><?php echo $v['size'] ?></option>
                <?php endforeach; ?>
              <?php endif; ?>
            </select>
          </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      function checkPrice() {
        $('#price').text(parseFloat($('#size_id').find(':selected').data('price')).toFixed(2));
      }
